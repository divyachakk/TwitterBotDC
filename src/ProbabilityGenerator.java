...
public class ProbabilityGenerator<String> extends TwitterBotMain { // generic class for the Probability Generator
	...
	ArrayList<Integer> alphabet_counts;// this is an ArrayList for keeping track of/counting how many times each unique
										// pitch occurs
	...
	void train(ArrayList<String> newTokens) {
		for (int i = 0; i < newTokens.size(); i++) { // parsing through each note in newTokens.size(), which is the
														// total ArrayList size
			// list of alphabet to find newTokens in the alphabet
			int index = alphabet.indexOf(newTokens.get(i)); // setting index equal to the index of newTokens.get(i) in
			...
			if (index == -1) { // if index doesn't exist in alphabet
				...
				alphabet_counts.add(0);// add zero to the alphabet counts array if particular newTokens(i) doesn's
										// exist
				index = alphabet.size() - 1;// put index value at the end of alphabet array
			}

			alphabet_counts.set(index, alphabet_counts.get(index) + 1);// move along alphabet_counts array while adding
																		// index value
		}
	...
	void printProbability() {
		for (int i = 0; i < alphabet.size(); i++) {
			System.out.println("Token: " + alphabet.get(i) + " | Probability: " + alphabet_counts.get(i) / sum); // this
																													// prints
																													// out
																													// the
																													// token
	...
	String generate() {
		...
		for (float ac : alphabet_counts) { // this adds the values of normalized probability distribution to an
											// ArrayList - probs
			probs.add(ac / sum);
		}

		for (int i = 0; i < probs.size() - 1; i++) { // this adds the values of probs, with current and previous value
														// summed together, to the ArrayList sumProbs
			filler += probs.get(i);
			sumProbs.add(filler);
		}
		sumProbs.add((float) 1); // since we're starting behind from 0, have to add one after the loop has been
									// executed
		...
		newToken = alphabet.get(i - 1); // newToken, after going through the while loop, then returns a value from the
										// alphabet ArrayList
